...
def process_country(country, data_dict):
    ...
    number_of_new_deseases = [0] + list(np.diff(number_of_deseases))

    # calculate doubling rate
    rate = np.divide(number_of_deseases[1:], number_of_deseases[:-1])

    for i in range(len(rate)):
        if rate[i] == 1.0:
            rate[i] = 100000000  # force doubling rate to be 0

    log2_vector = np.full((len(rate)), np.log(2))
    doubling_rate = [0.0] + list(np.divide(log2_vector, np.log(rate)))
...
def main():
    ...
    filedata = urlopen(csv_source)
    csvfile = StringIO(filedata.read().decode("utf8"))
    dict_reader = DictReader(csvfile, delimiter=',')
    if len(sys.argv) < 2:
        countries = ['Austria', 'Italy', 'Germany', 'Spain', 'Turkey', 'Russia']
    else:
